1. Покрыть всё тестами, использовать slogassert в тестах
2. Поправить pre-commit хук
...
4. Обновить README
   ...
   4.3 Проверь runme.sh для Mac. При его запуске ошибка, что-то с переносами не то.
   С дистрибутивом для MacOS что-то не то.

   4.4 Добавить описание порядка запуска приложения с перенаправлением вывода в stdout и stderr
   4.5 Добавить для Linux/Mac юзеров, что приложение во время работы создаёт файлы и папки с определенными правами.

5. Обновить CHANGELOG
6. Обновить скрипты сборки, использовать GoReleaser, исключить исходники из бинарных сборок для Mac/Linux.
7. В go-releaser сгенерировать хэши для всех архивов.
